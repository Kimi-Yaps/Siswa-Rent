...
const Favorites = () => {
  ...
  useEffect(() => {
    const fetchFavorites = async () => {
      try {
        ...
        // Supabase provides dot syntax for foreign key joins
  ...
  if (loading) {
    return <main className="favorites-page"><div className="loading-spinner">Loading...</div></main>;
  }
      ...
      <div className="favorites-container">
        ...
        {favorites.length === 0 && !error ? (
          <div className="empty-favorites">
            ...
            <Link to="/housing" className="browse-link">Browse Housing</Link>
          </div>
        ) : (
          <div className="favorites-grid">
            {favorites.map((fav) => {
              const property = fav.properties;
              // Provide a fallback if property is deleted but favorite remains
              if (!property) return null; 

              const coverImage = property.image_urls && property.image_urls.length > 0 
                ? property.image_urls[0] 
                : null; // Assuming fallback is handled in CSS or via a placeholder image

              return (
                <div key={fav.id} className="favorite-card">
                  <Link to={`/details/${property.id}`} className="favorite-image-container">
                    {coverImage ? (
                      <img src={coverImage} alt={property.name} className="favorite-image" />
                    ) : (
                  ...
                  <div className="favorite-content">
                    <div className="favorite-header">
                      <Link to={`/details/${property.id}`} className="favorite-name">
                        {property.name}
                      </Link>
                      <button 
                      ...
                      {editingId === fav.id ? (
                        <div className="notes-editor">
                          ...
                          <div className="notes-actions">
                            <button className="cancel-note" onClick={() => setEditingId(null)}>Cancel</button>
                            <button className="save-note" onClick={() => handleSaveNote(fav.id)}>Save</button>
                          </div>
                        </div>
                      ) : (
